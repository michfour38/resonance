...
export default function HarmonizeRecognitionPage({
  params,
}: {
  ...
  async function saveEntry() {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Unable to save recognition")
      }
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Something went wrong saving your recognition.",
      )
    } finally {
      setSaving(false)
    }
    ...
    <main className="min-h-screen bg-[#0b0b0b] text-[#f4f1ea]">
      <section className="mx-auto flex min-h-screen max-w-3xl flex-col justify-center px-6 py-20">
        ...
        <p className="mt-6 text-base leading-7 text-[#d8d2c6]">
          This is where Harmonize begins moving from what happened into what is
          becoming visible. No shared repair begins yet.
        </p>

        <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
          <p className="text-xl leading-8 text-[#f4f1ea]">
            As you reflect on this, what are you noticing about where you go
            inside?
          </p>
