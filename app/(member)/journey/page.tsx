...
type JourneyContentLike = {
  weekTitle?: string | null;
  weekNumber?: number | null;
  dayNumber?: number | null;
  weekTheme?: string | null;
  prompts?: PromptLike[] | null;
};
...
export default async function JourneyPage({
  ...
  let content: JourneyContentLike | null = null;
  let journeyLoadFailed = false;

  try {
    const result = await getCurrentDayContent(userId);
    content = (result ?? null) as JourneyContentLike | null;
  } catch (error) {
    journeyLoadFailed = true;
    console.error("Journey page failed to load current day content:", error);
  }

  if (journeyLoadFailed) {
    return (
      <JourneyShell>
        <div className="rounded-3xl border border-zinc-800/90 bg-black/55 p-6 backdrop-blur-[2px]">
          ...
          <p className="mt-4 text-base leading-7 text-zinc-300">
            Your access is working, but today’s Journey content could not be
            loaded yet.
          </p>

          <p className="mt-4 text-sm leading-7 text-zinc-400">
            This means the page stayed stable instead of crashing, and the next
            step is to inspect the Journey content service directly.
          </p>
  ...
  return (
    <JourneyShell>
      <div className="rounded-3xl border border-zinc-800/90 bg-black/55 p-6 backdrop-blur-[2px]">
        ...
        <h1 className="mt-4 text-3xl font-semibold text-white">
          {content.weekTitle || "Your Journey"}
        </h1>
        ...
        <p className="mt-4 text-base leading-7 text-zinc-300">
          {content.weekTheme || "Your guided reflection journey is active."}
        </p>
